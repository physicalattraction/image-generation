...
from tensorflow.python.framework.ops import get_default_graph, name_scope
from tensorflow.python.ops.array_ops import placeholder
from tensorflow.python.ops.variable_scope import AUTO_REUSE, get_variable_scope
...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        with name_scope(None):
            self.scope = get_default_graph().unique_name(self.name, mark_as_used=True)
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=AUTO_REUSE), tfutil.absolute_name_scope(self.scope):  # ignore surrounding scopes
            assert get_variable_scope().name == self.scope
            assert get_default_graph().get_name_scope() == self.scope
            with tf.control_dependencies(None):  # ignore surrounding control dependencies
                self.input_templates = [placeholder(tf.float32, name=name) for name in self.input_names]
